...
window.addEventListener('keydown', (e) => {
    switch (e.key) {
        case 'ArrowUp':
            postac.style.top = parseInt(postac.style.top) - moveBy + '%';
            break;
        case 'ArrowDown':
            postac.style.top = parseInt(postac.style.top) + moveBy + '%';
            break;
...
function egg_down(egg) {
    egg_current_position = parseInt(egg.css('right'));
    egg.css('right', egg_current_position + speed);
}
...
function set_egg_to_initial_position(egg) {
    egg.css('right', egg_initial_position);
}
